...
import gensim
model = gensim.models.Word2Vec.load(os.path.dirname(os.getcwd())+'/2_word2vec/cnki_wiki.model')
...
#         embedding_size=EMBEDDING_SIZE, name='words')
    word_vectors = np.zeros([len(X_train),80,150])
    for i in range(len(X_train)):
        for j in range(MAX_DOCUMENT_LENGTH):
            try:
                word_vectors[i][j][:] = model[vocab_processor.vocabulary_.reverse(X_train[i][j])]
            except KeyError:
                continue
            except TypeError:
                continue
